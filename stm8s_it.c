...
/**
  ...
  * @retval None
  */
extern u8 UART_Rx_Buff[];
extern uint16_t ADC_Mesh_Val,RD_Mesh_Opcode;
BitStatus receive_flag=0;
u8 i=0;
extern RD_Mesh_2_stm8_typedef *pointer,RD_Mesh_receive_data;
 #define RD_mesh_opcode 0x4C82   // ban tin set lightness cua den CCT
 INTERRUPT_HANDLER(UART1_RX_IRQHandler, 18)
 {
    if(UART1_GetITStatus(UART1_IT_RXNE)){
      UART_Rx_Buff[i]= UART1_ReceiveData8();
      i++;
      if(i>=4){
      RD_Mesh_Opcode =  (UART_Rx_Buff[0]<<8)|UART_Rx_Buff[1];
      if(RD_Mesh_Opcode==RD_mesh_opcode)
      {
        ADC_Mesh_Val = (UART_Rx_Buff[3]<<8)|UART_Rx_Buff[2];
        pointer = (RD_Mesh_2_stm8_typedef *) UART_Rx_Buff;
        RD_Mesh_receive_data = *pointer;
      }
      i=0;
      receive_flag=1;
      }
